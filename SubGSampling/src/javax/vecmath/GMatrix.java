...
public class GMatrix implements java.io.Serializable, Cloneable {
	 ...
	 *            number of columns in this matrix.
	 * @param arg
	 *            parameter value. Extended by yinxuse@gmail.com
	 */
	public GMatrix(int nRow, int nCol, double arg) {
		values = new double[nRow][nCol];
		this.nRow = nRow;
		this.nCol = nCol;
	...
	/**
	 * Sets the value of this matrix to the values found in the array parameter.
	 * and you should specify the Row you want to set.
	 * 
	 * @param matrix
	 *            the row major source array
	 * @param sRow
	 *            the row you want to set Extended by yinxusen@gmail.com
	 */
	public final void set(double[] matrix, int sRow) {
		assert (matrix.length == this.nCol);
		assert (sRow < this.nRow);
		int i;
		for (i = 0; i < this.nCol; ++i) {
			this.values[sRow][i] = matrix[i];
		}
	}

	/**
	 * Sets the value of a specified row and col to the value in parameter.
	 * 
	 * @param val
	 *            the value you want to set
	 * @param sRow
	 *            the row you want to set
	 * @param sCol
	 *            the col you want to set Extended by yinxusen@gmail.com
	 */
	public final void set(double val, int sRow, int sCol) {
		assert (sRow < this.nRow && sCol < this.nCol);
		this.values[sRow][sCol] = val;
	}
	...
	private static void update_v_split(int topr, int bottomr, GMatrix v,
		...
		if (debug) {
			...
			for (j = 0; j < v.nRow; j++) {
				...
				t.values[j][topr] = cosr[0] * vtemp - sinr[0]
						* t.values[j][bottomr];
				t.values[j][bottomr] = sinr[0] * vtemp + cosr[0]
						* t.values[j][bottomr];
			}
		}
	...
	private static void update_u_split(int topr, int bottomr, GMatrix u,
		...
		if (debug) {
			...
			for (j = 0; j < u.nCol; j++) {
				...
				t.values[topr][j] = cosl[0] * utemp - sinl[0]
						* t.values[bottomr][j];
				t.values[bottomr][j] = sinl[0] * utemp + cosl[0]
						* t.values[bottomr][j];
			}
		}
		System.out.println("\nm=");
	...
	/**
	 * 将所有的数据转换成 log 形式，避免乘除法运算浮动过大 by yinxs
	 */
	...
	/**
	 * 将所有转换成 log 形式的数据转换回来。 注意，一定要在上面那个用完才能用下面这个 转化数据之后，只能使用带 _log
	 * 的函数做运算，否则后果自负 by yinxs
	 */
	...
	/**
	 * 矩阵乘法运算 此函数为 log 运算函数，确保传入的矩阵均为log形式
	 * 
	 * @param m1
	 *            the first matrix
	 * @param m2
