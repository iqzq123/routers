...
public class NMFTest {
	...
	static public void testSample(String f, int turn) throws IOException {
		double sumQ = 0.0;
		for (int i = 0; i < turn; i++) {
			...
			e.setClusterFlie("c:/cluster.txt");
			e.setGraphFile(f);
			
			sumQ += e.getModularity();
	...
	static public void testSample(BufferedWriter writer, double outRatio,
		...
		for (int i = 0; i < turn; i++) {
			...
			CommunityGenerator generator = new GNCommunityGraphGenerator(
					communityNumber, communityNode, avgDegree, inDegreeRate);
			...
			double score1 = similarity1.getSimilarity(graph, topicCommunities,
					standardCommunities);
			double score2 = similarity2.getSimilarity(graph, topicCommunities,
					standardCommunities);
			double score3 = dongenMetric.getSimilarity(graph, topicCommunities,
					standardCommunities);
			...
			double module = gnModularity.getQualityScore(graph,
					topicCommunities);
			sumQ += module;

			GlobalQualityMetric gnModularity2 = new NGModularity();
			double module2 = gnModularity2.getQualityScore(graph,
					standardCommunities);
			sumStandQ += module2;
			// System.out.println("Score" + score);
		}
		...
		sumQ /= turn;
		sumStandQ /= turn;
		System.out.println(outRatio + "\t" + nmiScore + "\t" + jaccardIndex
				+ "\t" + dongenScore + "\t" + sumQ);
		writer.write(outRatio + "\t" + nmiScore + "\t" + jaccardIndex + "\t"
				+ dongenScore + "\n");
		writer.flush();
	}

	static public void getCommunities(BufferedWriter writer, double outRatio,
			int turn) throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= 1; i++) {
			double out = i * 0.5;
			NMFTest.testSample("D:/data/coPaper.txt", 3);
		}
		writer.close();
	}
}
