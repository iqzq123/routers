...
public class LdaGibbsSampler {
	...
	public void test() throws IOException {
		...
		FileInputStream file = new FileInputStream(new File("c:/subgraphs.txt"));
		...
		while ((tempString = reader.readLine()) != null) {
			line++;
			docString.add(tempString);
		}
		int[][] documents = new int[line][];
		int cnt = 0;
		int max = -1;
		HashSet wordset = new HashSet();
		for (String docStr : docString) {
			String[] docArray = docStr.split("\\s");
			documents[cnt] = new int[docArray.length];
			for (int i = 0; i < docArray.length; i++) {
				int a = Integer.parseInt(docArray[i]);
				if (a > max) {
					max = a;
				}
				documents[cnt][i] = a;
				wordset.add(documents[cnt][i]);
			}
			cnt++;
		}
		// vocabulary
	...
	public static void main(String[] args) throws IOException {
		...
		LdaGibbsSampler l = new LdaGibbsSampler();
		l.test();
