...
public class RLSI {
	...
	public static void main(String[] args) throws Exception {
		...
		r.readData("c:/mdt.txt","c:/data/term.txt");
	...
	public void readData(String dtFile,String dictFile ) throws IOException {
		...
		while ((tempString = reader.readLine()) != null) {
			...
			doc[docId].add(new DTIndex(termId, weight));
			this.invertedIndex[termId].add(new TDIndex(docId, weight));
		}
		reader.close();
		FileInputStream df = new FileInputStream(new File(dictFile));
		System.out.println("以行为单位读取文件内容，一次读一整行：");
		reader = new BufferedReader(new InputStreamReader(df, "UTF-8"));
...
//				}
				for(DTIndex t:this.doc[n]){
					product+=t.weight*this.wordTopic[t.termId][k];
				}
				O[k][n]=product;
			}
		}
	...
	public void gradientSolveV(int n) {
		...
		while (trainNum-- > 0) {
			...
			for (DTIndex t : this.doc[n]) {
				error[t.termId] = t.weight - Uv[t.termId];
				Uv[t.termId] = 0.0;
			}
			for (int m = 0; m < M; m++) {
